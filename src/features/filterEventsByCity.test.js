...
defineFeature(feature, (test) => {
  ...
  test("User should see a list of suggestions when they search for a city.", ({
    ...
    then(
      ...
      async () => {
        ...
        expect(suggestionListItems).toHaveLength(2); //When use types Berlin, you should see "Berlin, Germany" and "See all Cities"
      }
    );
  });
  ...
  test("User can select a city from the suggested list.", ({
    ...
    when(
      ...
      async () => {
        const user = userEvent.setup();
        await user.click(suggestionListItems[0]); //“Berlin, Germany” should be first on the list
      }
    );
